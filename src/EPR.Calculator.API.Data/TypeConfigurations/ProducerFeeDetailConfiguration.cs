...
public class ProducerFeeDetailConfiguration : IEntityTypeConfiguration<ProducerFeeDetail>
{
    public void Configure(EntityTypeBuilder<ProducerFeeDetail> builder)
    {
        builder.ToTable("calc_result_producer_fee_detail");

        builder.Property(x => x.Id)
            .HasColumnName("id")
            .IsRequired();

        builder.Property(x => x.ProducerFeesId)
            .HasColumnName("producer_fees_id")
            .IsRequired();

        builder.HasOne<ProducerFees>()
            .WithMany(p => p.Details)
            .HasForeignKey(x => x.ProducerFeesId);

        builder.OwnsOne(x => x.FeeDetail, fd =>
        {
            fd.ToJson("detail");
            ConfigureFeeDetail(fd);
        });
    }

    public static void ConfigureFeeDetail<TOwner>(
        OwnedNavigationBuilder<TOwner, FeeDetail> b) where TOwner : class
    {
