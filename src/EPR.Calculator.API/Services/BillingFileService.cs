...
namespace EPR.Calculator.API.Services
{
    ...
    public class BillingFileService(ApplicationDBContext applicationDBContext, IBlobStorageService2 blobStorageService2, IConfiguration configuration) : IBillingFileService
    {
            ...
            var baseQuery = applicationDBContext.ProducerResultFileSuggestedBillingInstruction
                ...
                .Select(prsi => new
                {
                    ...
                    PendingStatus = (string.IsNullOrWhiteSpace(prsi.SuggestedBillingInstruction) || prsi.SuggestedBillingInstruction.Trim() == NoActionPlaceholder)
                        ? Enums.BillingInstruction.Noaction.ToString()
            ...
            CancellationToken cancellationToken)
        {
            ...
            response.TotalInitialRecords = groupedBillingInstructionResult.Find(s => string.Equals(s.Suggestion, Enums.BillingInstruction.Initial.ToString(), StringComparison.OrdinalIgnoreCase))?.TotalRecords ?? 0;
            response.TotalDeltaRecords = groupedBillingInstructionResult.Find(s => string.Equals(s.Suggestion, Enums.BillingInstruction.Delta.ToString(), StringComparison.OrdinalIgnoreCase))?.TotalRecords ?? 0;
            response.TotalRebillRecords = groupedBillingInstructionResult.Find(s => string.Equals(s.Suggestion, Enums.BillingInstruction.Rebill.ToString(), StringComparison.OrdinalIgnoreCase))?.TotalRecords ?? 0;
            response.TotalCancelBillRecords = groupedBillingInstructionResult.Find(s => string.Equals(s.Suggestion, Enums.BillingInstruction.Cancel.ToString(), StringComparison.OrdinalIgnoreCase))?.TotalRecords ?? 0;
            response.TotalNoActionRecords = groupedBillingInstructionResult.Find(s => string.Equals(s.Suggestion, Enums.BillingInstruction.Noaction.ToString(), StringComparison.OrdinalIgnoreCase))?.TotalRecords ?? 0;
        }
    }
}
